...
const renderCurrentlyWeather = (currentData) => {
  let temperature = utils.fToC(currentData.temperature),
    ...
    percentages = [],
    precipProbability = utils.decimalToPercentage(
  ...
  document.getElementById("windGust").innerHTML = `${Math.round(
    windGust
  )} km/h`;
...
const renderUvIndex = (uvIndex) => {
  let content = document.getElementById("uvIndexBadges"),
    badges = [],
  ...
  badges.forEach((badge, index) => {
    ...
    if (uvIndex <= 2 && index == 0)
      badge.classList.replace("badge-secondary", "badge-success");
    else if (uvIndex >= 3 && uvIndex <= 5 && index == 1)
      badge.classList.replace("badge-secondary", "badge-caution");
    else if (uvIndex >= 6 && uvIndex <= 7 && index == 2)
      badge.classList.replace("badge-secondary", "badge-warning");
    else if (uvIndex >= 8 && uvIndex <= 10 && index == 3)
      badge.classList.replace("badge-secondary", "badge-danger");
    else if (uvIndex >= 11 && index == 4)
      badge.classList.replace("badge-secondary", "badge-alarming");
  });
  for (let i = 0; i < 5; i++) {
...
const renderTempChart = (dailyData) => {
  let timeTab = [],
    tempTab = [],
    rowcount = dailyData.data.length;
...
const renderWeeklyForecast = (weeklyForecastData) => {
  ...
  for (let i = 0; i < rowcount; i++) {
    let timestamp = time.formatTimestamp(weeklyForecastData.data[i].time),
      ...
      tempHigh = `${Math.round(
        utils.fToC(weeklyForecastData.data[i].temperatureHigh)
      )}&degC`,
      tempLow = `${Math.round(
        utils.fToC(weeklyForecastData.data[i].temperatureLow)
      )}&degC`;
    ...
    document
      ...
      .setAttribute("alt", `${weeklyForecastData.data[i].icon} icon`);
    document.getElementById(
      "tempHigh" + (i + 1)
    ).innerHTML = `<span class="mr-1"><i class="fas fa-angle-up"></i></span>${tempHigh}`;
    document.getElementById(
      "tempLow" + (i + 1)
    ).innerHTML = `<span class="mr-1"><i class="fas fa-angle-down"></i></span>${tempLow}`;
  }
};

export {
  renderCurrentlyWeather,
  renderDailyWeather,
